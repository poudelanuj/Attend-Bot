import { Client, GatewayIntentBits, EmbedBuilder, ActionRowBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, REST, Routes, SlashCommandBuilder } from 'discord.js';
...
async function handleCheckOut(interaction) {
  try {
    ...
    if (!todayAttendance || !todayAttendance.check_in_time) {
      await interaction.reply({ content: '❌ You haven\'t checked in today. Please check in first.', ephemeral: true });
      return;
    }
...
async function handleStatus(interaction) {
  try {
    ...
    if (stats) {
      embed.addFields(
          { name: '📈 30-Day Stats', value: `Days worked: ${stats.total_days}\nCompleted days: ${stats.completed_days}\nAverage rating: ${stats.avg_rating ? stats.avg_rating.toFixed(1) : 'N/A'}/5`, inline: true }
      );
    }
